...
export class GithubAuthService implements AuthService {
  ...
  async handleCallback(code: string, state: string): Promise<{ user: UserData; token: string }> {
    ...
    const user: UserData = userDataSchema.parse({
    ...
    const token = await new SignJWT(user).setProtectedHeader({ alg: "HS256" }).sign(this.encodedSecret);
